...
def forecast_next_value_wma(series):
    """ Predicts the next value using a Weighted Moving Average (WMA). """
...
def create_summary_chart(comparison_df):
    """ Creates a stacked bar chart displaying the total container count per source. """
    ...
    try:
        ...
        st.warning("Could not create summary chart because data is missing.")
        return
...
def extract_date_from_edi_content(edi_content):
    """
    Extracts the date from EDI file content by searching for the DTM+137:YYMMDD line.
    """
...
st.caption("Upload EDI files to compare, forecast, and validate cargo for the next vessel.")
...
with st.sidebar:
    ...
    if uploaded_files:
        ...
        for f in uploaded_files:
            ...
            # If filename parsing fails, try parsing from file content
            if not file_date:
                try:
                    f.seek(0)
                    ...
                    st.warning(f"Error reading file content for '{f.name}': {e}")
            
            if file_date:
                file_dates[f.name] = file_date
            else:
                file_dates[f.name] = datetime.today().date()
                st.warning(f"Could not extract date from filename or file content for '{f.name}'. Using today's date as fallback.")
        
        # Display extracted dates for debugging
        with st.expander("Extracted File Dates"):
            st.write(file_dates)

        sorted_file_names = sorted(file_dates, key=file_dates.get)
        selected_files = st.multiselect("2. Select files (sorted sequentially by date):", options=sorted_file_names, default=sorted_file_names)
...
if not uploaded_files or len(uploaded_files) < 2:
    ...
    st.warning("Please select at least 2 historical files in the sidebar to display a comparison.")
else:
    ...
    if comparison_df.empty:
                    ...
                    with cols[col_idx % 3]:
                        with st.container(border=True):
                            ...
                            for size in ['20', '40']:
                                ...
                                for pod in size_data['Port of Discharge'].unique():
                                    pod_data = size_data[size_data['Port of Discharge'] == pod]
                                    
                                    under_15 = pod_data[pod_data['Forecast Count'] < 15]
                                    over_15 = pod_data[pod_data['Forecast Count'] >= 15]
                                    
                                    slots_under_15 = np.ceil(under_15['Forecast Count'].sum() / 30)
                                    slots_over_15 = np.ceil(over_15['Forecast Count'] / 30).sum()
                                    
                                    total_pod_slots = slots_under_15 + slots_over_15
                                    if size == '40':
                                        total_pod_slots *= 2
                                    total_slots += total_pod_slots
                            
                            total_boxes = alloc_df['Forecast Count'].sum()
                            
                            m_col1, m_col2 = st.columns(2)
                            m_col1.metric("Total Forecasted Boxes", f"{total_boxes:,.0f}")
                            m_col2.metric("Total Slot Requirement", f"{total_slots:,.0f}")
        ...
        with st.expander("Show Comparison & Detailed Forecast Table"):
